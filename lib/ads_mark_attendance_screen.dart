...
class ADSMarkAttendanceScreen extends StatefulWidget {
  final String meetId;
  final Map<String, dynamic> adsData;

  const ADSMarkAttendanceScreen({
    super.key, 
  ...
  @override
  State<ADSMarkAttendanceScreen> createState() => _ADSMarkAttendanceScreenState();
}

class _ADSMarkAttendanceScreenState extends State<ADSMarkAttendanceScreen> {
  ...
  Future<void> _fetchData() async {
    // Assuming the ADS secretary's ward defines their jurisdiction
    final adsWard = widget.adsData['ward']?.toString() ?? '';
    ...
    try {
      ...
      // 2. Fetch Members: ONLY NHG Secretaries (and within the ADS's ward if applicable)
      var query = supabase.from('Registered_Members')
          .select('full_name, aadhar_number')
          .eq('designation', 'NHG_SECRETARY');
          
      // Filter by ward so they only see NHG secretaries in their area
      if (adsWard.isNotEmpty) {
        query = query.eq('ward', adsWard);
      }

      final membersResponse = await query.order('full_name', ascending: true);
  ...
  Future<void> _submitAttendance() async {
    ...
    try {
      ...
      // Update meeting status to closed/HELD
  ...
  Widget build(BuildContext context) {
    ...
    // Using the ADS Primary Color
    const Color primaryColor = Color(0xFF2B6CB0);

    return Scaffold(
      backgroundColor: const Color(0xFFF4F7F6),
      appBar: AppBar(
        title: const Text("ADS Attendance Hub", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), 
        ...
        children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    _isLocationSet 
                      ? "NHG Secretaries can now use 'Mark Attendance' on their phones."
          ...
          Expanded(
            child: _members.isEmpty 
              ? const Center(child: Text("No NHG Secretaries found in your ward.", style: TextStyle(color: Colors.grey)))
              : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: CheckboxListTile(
                    ...
                    subtitle: Text("ID: $aadhar • NHG Secretary", style: const TextStyle(fontSize: 11)),
                    value: isPresent,
